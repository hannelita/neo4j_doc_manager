...
class NodesAndRelationshipsUpdater(object):
  ...
  def run_update(self, update_spec, doc_id, doc_type):
    ...
    for spec in update_spec.keys():
      if spec=='$set':
        ...
        statement = "MATCH (d:Document:`%(doc_type)s`) WHERE d._id={doc_id} SET d+={set_parameter}" % locals()
        self.statements_with_params.append({statement: params_dict})
      elif spec=='$unset':
        update_value_list = update_spec['$unset']
        for update_value in update_value_list.keys():
          statement = "MATCH (d:Document:`%(doc_type)s`), (c:Document:`%(update_value)s`) WHERE d._id={doc_id} OPTIONAL MATCH (d)-[r]-(c) DELETE r WITH d, c OPTIONAL MATCH (c)-[s]-() WITH d,c,s, CASE WHEN s IS NULL THEN c ELSE NULL END AS n DELETE n" % locals()
          self.statements_with_params.append({statement: params_dict})
          statement = "MATCH (d:Document:`%(doc_type)s` { _id: {doc_id} } ) REMOVE d.`%(update_value)s` " % locals()
          self.statements_with_params.append({statement: params_dict})
      else:
  ...
  def remove_legacy_nodes(self, doc_id, doc_type):
    params_dict = {"doc_id": doc_id}
    statement = "MATCH (d:Document:`%(doc_type)s`) WHERE d._id={doc_id} OPTIONAL MATCH (d)-[r]-(c) DELETE r WITH d, c OPTIONAL MATCH (c)-[s]-() WITH d,c,s, CASE WHEN s IS NULL THEN c ELSE NULL END AS n DELETE n" % locals()
    self.statements_with_params.append({statement: params_dict})

  def clear_node(self, doc_type, doc_id):
    params_dict = {"doc_id": doc_id}
    params_dict.update({"parameters": {"_id": doc_id}})
    statement = "MATCH (d:Document:`%(doc_type)s`) WHERE d._id={doc_id} SET d={} SET d={parameters}" % locals()
    self.statements_with_params.append({statement: params_dict})
